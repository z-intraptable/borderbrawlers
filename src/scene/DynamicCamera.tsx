...
const IDLE_HALF_WIDTH = STAGE_HALF_WIDTH + 1.5;
...
const PAN_LIMIT_X = STAGE_HALF_WIDTH * 0.45;
const PAN_LIMIT_Y_MIN = STAGE_FLOOR_Y - 1;
const PAN_LIMIT_Y_MAX = STAGE_FLOOR_Y + 6;
...
export function DynamicCamera({ focus, enabled = true }: DynamicCameraProps): null {
  ...
  useFrame((_, delta) => {
    ...
    if (focus.count > 0) {
      ...
      needHalfW = Math.max((focus.maxX - focus.minX) / 2 + PADDING, IDLE_HALF_WIDTH * 0.45);
      needHalfH = Math.max((focus.maxY - focus.minY) / 2 + PADDING, IDLE_HALF_HEIGHT * 0.45);
    } else {
