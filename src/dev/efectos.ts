...
const CASILLEROS: { titulo: string; tirar(x: number, y: number): void }[] = [
  ...
  },
  {
    ...
    tirar: (x, y) => {
      ...
      ring(fx, x, y, 2.6, 0.44, GREEN);
      slash(fx, x + 0.35, y, 0, 1.25, 0.24, GREEN);
      burst(fx, x, y, 10, 6.5, 0.1, 0.42, GREEN);
    },
  },
  {
    titulo: 'habilidad B',
    tirar: (x, y) => {
      flash(fx, x, y, 0.62, 0.16, 0xffffff);
      ring(fx, x, y, 2.1, 0.44, RED);
      shards(fx, x, y, 7, 5.5, RED);
  ...
  },
  {
    titulo: 'KO',
    tirar: (x, y) => {
      flash(fx, x, y, 0.95, 0.22, 0xffffff);
      shards(fx, x, y, 10, 8, GREEN);
      burst(fx, x, y, 14, 9, 0.16, 0.8, 0xffffff);
      ring(fx, x, y, 2.6, 0.55, GREEN);
    },
  },
  {
